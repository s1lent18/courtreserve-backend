...
public class TournamentServiceImpl implements TournamentService {
        ...
        public PaginatedResponse<GetTournamentResponse> getAllTournaments(String location, Pageable pageable) {
                ...
                Page<Tournament> tournaments = tournamentRepository.findAllByCourt_Location(location, pageable);
        ...
        @Override
        public TournamentResponse hostedTournament(Long userId) {
                User user = userRepository.findById(userId)
                                .orElseThrow(() -> new ResourceNotFoundException("User", "id", userId));

                Tournament tournament = tournamentRepository.findByOrganizer_Id(user.getId());
                ...
                for (Long teamId : teamIds) {
                        ...
                        if (!team.getSport().equalsIgnoreCase(tournament.getSport())) {
                                throw new BadRequestException(
                                                "Team '" + team.getName() + "' sport does not match tournament sport");
                        }

                        // Check if team is already registered
                        if (tournamentTeamRepository.existsByTournamentAndTeam(tournament, team)) {
                                throw new ConflictException("Team '" + team.getName()
